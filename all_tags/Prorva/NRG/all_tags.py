...
try:
    ...
    A = open('/webapps/cmv2/all_tags/Prorva/NRG/tags.txt', 'r')
    ...
    for i in Data:
        ...
        quan = cur.execute("SELECT * FROM n_wincctags where tag_key='" + tag + "' and oil_field = 'NRG'")
        if (quan == 0):
            Sql = (
                "INSERT INTO n_wincctags(oil_field, tag_key, tag_value, last_update) VALUES('NRG','{0}','{1}',now())").format(
                tag, value)
            # print(Sql)
            # if(quality=='Good'):
            cur.execute(Sql)
            ...
            Sql = (
                "UPDATE n_wincctags SET tag_value='{0}',last_update=now(),last_actual_update='{1}' WHERE tag_key='{2}' and oil_field='NRG'").format(
    ...
    conn.close()
    opc.close()
    C = open('/webapps/cmv2/all_tags/Prorva/NRG/numberOfRetries.txt', 'r')
    ...
    B = open("/webapps/cmv2/all_tags/Prorva/NRG/log.txt", "a")
    C = open('/webapps/cmv2/all_tags/Prorva/NRG/numberOfRetries.txt', 'r')
    numberOfRetries = int(C.read())
    C.close()
    ...
    if (numberOfRetries > 20):
        smtpObj.sendmail('noreply@dlc.kz', send_to, 'Timeout occured or opc server(NRG) is unavailable')
        numberOfRetries = 0
    C = open('/webapps/cmv2/all_tags/Prorva/NRG/numberOfRetries.txt', 'w')
