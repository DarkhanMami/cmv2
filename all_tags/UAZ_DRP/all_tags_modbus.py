...
from all_tags.UAZ_DRP import tags_modbus


def update_tags():
    c = ModbusClient(host="192.168.241.217", port=502, auto_open=True)

    conn = pymysql.connect('192.168.17.158', port=3306, user='root', passwd='1234', db='emg-cm')
    cur = conn.cursor()
    c.open()

    for tag in tags_modbus.tags:
        reg = tags_modbus.tags[tag]
        regs = None
        for i in range(5):
