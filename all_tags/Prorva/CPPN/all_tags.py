...
try:
    ...
    A = open('/webapps/cmv2/all_tags/Prorva/CPPN/tags.txt', 'r')
    ...
    for i in Data:
        ...
        quan = cur.execute("SELECT * FROM n_wincctags where tag_key='" + tag + "' and oil_field = 'CPPN'")
        if (quan == 0):
            Sql = (
                "INSERT INTO n_wincctags(oil_field, tag_key, tag_value, last_update) VALUES('CPPN','{0}','{1}',now())").format(
                tag, value)
            # print(Sql)
            # if(quality=='Good'):
            cur.execute(Sql)
            ...
            Sql = (
                "UPDATE n_wincctags SET tag_value='{0}',last_update=now(),last_actual_update='{1}' WHERE tag_key='{2}' and oil_field='CPPN'").format(
    ...
    conn.close()
    opc.close()
    C = open('/webapps/cmv2/all_tags/Prorva/CPPN/numberOfRetries.txt', 'r')
    ...
    B = open("/webapps/cmv2/all_tags/Prorva/CPPN/log.txt", "a")
    C = open('/webapps/cmv2/all_tags/Prorva/CPPN/numberOfRetries.txt', 'r')
    numberOfRetries = int(C.read())
    C.close()
    ...
    if (numberOfRetries > 20):
        smtpObj.sendmail('noreply@dlc.kz', send_to, 'Timeout occured or opc server(CPPN) is unavailable')
        numberOfRetries = 0
    C = open('/webapps/cmv2/all_tags/Prorva/CPPN/numberOfRetries.txt', 'w')
